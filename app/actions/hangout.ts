...
import { isPastIso, ymdSeoul } from "@/lib/calendar";
...
export async function createHangout(
  ...
  if (isPastIso(date, ymdSeoul(new Date()))) return { ok: false };
...
export async function updateHangout(
  ...
  // where에 creatorId 포함 = 소유권 검증 + 수정을 원자적 쿼리 1회로.
  // P2025 = 없거나 내 것 아님 → 실패로 알림.
  try {
    const { groupId } = await prisma.hangout.update({
...
export async function deleteHangout(formData: FormData) {
  ...
  // where에 creatorId 포함 = 소유권 검증 + 삭제를 원자적 쿼리 1회로.
  // P2025 = 이미 삭제됐거나 내 것 아님 → 무해, 조용히 종료.
  try {
    const { groupId } = await prisma.hangout.delete({
      where: { id: hangoutId, creatorId: user.id },
      select: { groupId: true },
    });
    revalidatePath(`/groups/${groupId}`);
